...
// Livelli di attivita' per il metodo Gerrior/Juan/Basiotis (CDC, Prev Chronic Dis 2006;3(4):A129):
// coefficiente PA specifico per sesso, verificato a cifra esatta contro l'esempio
// numerico del paper stesso (non la tabella IOM "da manuale" generica).
const ATTIVITA_EER = [
  { id: "sedentario_eer", nome: "Sedentario", desc: "Solo le attivita' della vita quotidiana, nessun esercizio aggiuntivo.", pa: { uomo: 1.0, donna: 1.0 } },
  { id: "poco_attivo_eer", nome: "Poco attivo", desc: "Attivita' quotidiane piu' 30-60 minuti di attivita' moderata al giorno.", pa: { uomo: 1.12, donna: 1.14 } },
  { id: "attivo_eer", nome: "Attivo", desc: "Attivita' quotidiane piu' oltre 60 minuti di attivita' moderata al giorno.", pa: { uomo: 1.27, donna: 1.27 } },
  { id: "molto_attivo_eer", nome: "Molto attivo", desc: "Oltre 60 minuti di attivita' moderata e 60 di vigorosa (o 120 di moderata) al giorno.", pa: { uomo: 1.54, donna: 1.45 } },
];
...
export default function MindbiteApp() {
  ...
  function caricaProfiloInStato(p) {
    ...
    setMetodoCalcolo(p.metodoCalcolo || "mifflin");
  ...
  const [metodoCalcolo, setMetodoCalcolo] = useState("mifflin"); // "mifflin" | "eer"
  ...
  const calc = useMemo(() => {
    ...
    if (metodoCalcolo === "eer") {
      const attivitaObj = ATTIVITA_EER.find((x) => x.id === attivita);
      if (!attivitaObj) return null;
      const pa = attivitaObj.pa[sesso];
      const hm = h / 100;
      tdee =
        sesso === "uomo"
          ? 864 - 9.72 * a + pa * (14.2 * p + 503 * hm)
          : 387 - 7.31 * a + pa * (10.9 * p + 660.7 * hm);
    } else {
      bmr = sesso === "uomo" ? 10 * p + 6.25 * h - 5 * a + 5 : 10 * p + 6.25 * h - 5 * a - 161;
  ...
  }, [peso, altezza, eta, sesso, attivita, obiettivo, ritmo, carbP, fatP, proteinP, metodoCalcolo]);
      ...
      <div className="kn-shell">
        ...
        {screen === "step2" && (
          <>
            ...
            <div className="kn-field">
              <label className="kn-label">Metodo di calcolo del fabbisogno</label>
              <div className="kn-toggle-row">
                <div className={"kn-toggle" + (metodoCalcolo === "mifflin" ? " sel" : "")} onClick={() => { setMetodoCalcolo("mifflin"); setAttivita(null); }}>Mifflin-St Jeor</div>
                <div className={"kn-toggle" + (metodoCalcolo === "eer" ? " sel" : "")} onClick={() => { setMetodoCalcolo("eer"); setAttivita(null); }}>IOM / EER</div>
              </div>
              {metodoCalcolo === "eer" && (
                <p className="kn-sub" style={{ marginTop: 8, marginBottom: 0, fontSize: 12 }}>
                  Metodo di Gerrior, Juan e Basiotis (CDC, <em>Prev Chronic Dis</em> 2006) — <a href="https://www.cdc.gov/pcd/issues/2006/oct/pdf/06_0034.pdf" target="_blank" rel="noreferrer" style={{ color: "var(--accent)" }}>consulta il paper originale</a>.
                </p>
              )}
            </div>

            <p className="kn-sub">Scegli il livello che descrive la tua settimana tipo, non quella ideale.</p>
            {(metodoCalcolo === "eer" ? ATTIVITA_EER : ATTIVITA).map((a) => (
        ...
        {screen === "profilo" && (
          <>
            ...
            <div className="kn-profile-section">
              ...
              <div className="kn-toggle-row" style={{ marginBottom: 14 }}>
                <div className={"kn-toggle" + (metodoCalcolo === "mifflin" ? " sel" : "")} onClick={() => { segna(setMetodoCalcolo)("mifflin"); segna(setAttivita)(null); }}>Mifflin-St Jeor</div>
                <div className={"kn-toggle" + (metodoCalcolo === "eer" ? " sel" : "")} onClick={() => { segna(setMetodoCalcolo)("eer"); segna(setAttivita)(null); }}>IOM / EER</div>
              </div>
              {metodoCalcolo === "eer" && (
                <p className="kn-sub" style={{ marginTop: -6, marginBottom: 14, fontSize: 12 }}>
                  Metodo di Gerrior, Juan e Basiotis (CDC, <em>Prev Chronic Dis</em> 2006) — <a href="https://www.cdc.gov/pcd/issues/2006/oct/pdf/06_0034.pdf" target="_blank" rel="noreferrer" style={{ color: "var(--accent)" }}>consulta il paper originale</a>.
                </p>
              )}
              {(metodoCalcolo === "eer" ? ATTIVITA_EER : ATTIVITA).map((a) => (
